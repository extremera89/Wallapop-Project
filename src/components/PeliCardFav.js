...
import { getDatabase, ref, child, get, set, push, update, remove, doc } from "firebase/database";

import { UsuarioContext } from './UsuarioContext';
import { ErrorMessage } from 'formik';


export const PeliCardFav = (element) => {
    ...
    const isFavorite = (snapshot) => {
        ...
        if (snapshot.exists()) {
            ...
            algo.map((pelicula) => {
                if(pelicula.nombre === element.element.nombre){
                    respuesta = true;
                }
        ...
        const dbRef = ref(getDatabase());
            ...
            if (snapshot.exists()) {
                if(isFavorite(snapshot) === true){
                    ...
                    //const algo = Object.entries(snapshot.val().favoritos).map(([k,v]) => {return v})
                    const elimino = ref(db, `users/${usuario.uid}/favoritos/${element.element.nombre}`)
                    remove(elimino).then(()=> {
        ...
        <div id="lista">
            ...
            <Card  id="card" sx={{ width: 180, justifyItems:"center", justifyContent: "center", textAlign: "center", borderRadius: 10}}>
                <CardActionArea  sx={{ height: 280, }}>
                    ...
                    className="fotopeli"
                    component="img"
                    image={`https://image.tmdb.org/t/p/original/${element.element.imagen}`}
            ...
            <div id="titulo">
                ...
                <div onClick={prueba} id="precio" className="precio">
                    <p> {element.element.precio}€</p>
                    <FavoriteBorderIcon id="favorito" className="favorito" ></FavoriteBorderIcon>
                </div>
